...
# ==========================================
# 🛠 ১. সিস্টেম ও ডিপেন্ডেন্সি (No Shortcutting)
# ==========================================
print("⚙️ System Initializing: Loading Core Modules...")
...
required_packages = [
    "pyrogram", "tgcrypto", "yt-dlp", "requests", 
    "bs4", "imageio-ffmpeg", "aiohttp", "fake-useragent", "cloudscraper"
]
...
# ==========================================
# 🌐 ৪. Render Web Server (Port Binding)
# ==========================================
...
async def update_progress(message, percentage, current, total, speed, status_text, task_id):
    ...
    if (now - LAST_UPDATE_TIME.get(msg_key, 0)) < 4: return
    LAST_UPDATE_TIME[msg_key] = now
...
# ==========================================
# 🕵️‍♂️ ৬. সুপার স্ক্র্যাপার (Deep Scanning Logic)
# ==========================================
def get_real_video_link(page_url):
    headers = {'User-Agent': UserAgent().random, 'Referer': page_url}
    try:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if not clean_link.startswith('http'): clean_link = urljoin(page_url, clean_link)
                found_links.append(clean_link)

        final_url = page_url
        if found_links:
            m3u8 = [l for l in found_links if '.m3u8' in l]
            final_url = m3u8[0] if m3u8 else found_links[0]

        return {'original_url': page_url, 'video_url': final_url, 'headers': headers}
    except:
        return {'original_url': page_url, 'video_url': page_url, 'headers': headers}

# ==========================================
# 🤖 ৭. বট হ্যান্ডলার (Start, Text, Callbacks)
# ==========================================
@app.on_message(filters.command("start"))
async def start_msg(c, m):
    await m.reply("👋 **Universal Downloader Pro!**\n\nযেকোনো ভিডিও লিঙ্ক পাঠান, আমি সেটি ডাউনলোড করে ফাইল হিসেবে পাঠিয়ে দেব।\n\n✅ Aria2c: Active\n✅ Deep Scraper: Active")
...
async def text_handler(client, message):
    ...
    status_msg = await message.reply("🕵️‍♂️ **লিঙ্ক এনালাইজ করছি...**")
...
async def cb_handler(client, query: CallbackQuery):
    ...
    if data.startswith("stop_"):
        tid = data.split("_")[1]
        CANCEL_EVENTS[tid] = True
        await query.answer("🛑 ক্যানসেল হচ্ছে...", show_alert=True)
        return

    if data.startswith("q_"):
        _, task_id, mode = data.split("_")
        if task_id not in TASK_STORE: return await query.answer("টাস্ক এক্সপায়ার হয়েছে!", show_alert=True)
...
async def run_download_upload(client, message, task_info, task_id, custom_name):
    async with semaphore:
        ...
        file_name = custom_name if custom_name else clean_filename(task_info['title'])
        ...
        def ydl_hook(d):
            ...
            if d['status'] == 'downloading':
                ...
                if total > 0:
                    ...
                    asyncio.run_coroutine_threadsafe(update_progress(message, percentage, current, total, speed, "⬇️ ডাউনলোড হচ্ছে", task_id), asyncio.get_event_loop())
        ...
        ydl_opts = {
            ...
            'http_headers': link_data['headers'],
            'noplaylist': True,
        }
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                await asyncio.to_thread(ydl.download, [link_data['video_url']])
            
            files = os.listdir(temp_dir)
            if not files: raise Exception("ফাইল খুঁজে পাওয়া যায়নি!")
            
            # Find the main file and thumbnail
            final_path = ""
            thumb_path = None
            for f in files:
                if f.endswith((".mp4", ".mkv", ".mp3", ".webm")): final_path = os.path.join(temp_dir, f)
                if f.endswith((".jpg", ".png", ".webp")): thumb_path = os.path.join(temp_dir, f)

            file_size = os.path.getsize(final_path)
            if file_size > 2000 * 1024 * 1024: return await message.edit("❌ ফাইলটি ২ জিবির বেশি!")

            await message.edit("📤 **টেলিগ্রামে আপলোড হচ্ছে...**")
            
            async def upload_progress(current, total):
                if CANCEL_EVENTS.get(task_id): client.stop_transmission()
                await update_progress(message, (current/total)*100, current, total, current/(time.time()-start_time), "⬆️ আপলোড হচ্ছে", task_id)

            caption = f"📁 **নাম:** `{file_name}`\n💾 **সাইজ:** `{human_readable_size(file_size)}`"
            
            if mode == 'doc':
                await client.send_document(message.chat.id, final_path, caption=caption, thumb=thumb_path, progress=upload_progress)
            else:
...
# ==========================================
# 🔥 ৯. ফাইনাল এক্সিকিউশন (Render Support)
# ==========================================
